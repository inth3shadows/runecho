...
### Changed
- guard: `RUNECHO_GUARD_QUALIFIED` (Go same-repo qualified-call check) is now **default ON** — `RUNECHO_GUARD_QUALIFIED=0` to disable. The compiler-oracle differential now measures the check for false positives (it previously measured only reach): 0 proven false positives over this repo and 472,938 lines of `golang.org/x/text`. `RUNECHO_GUARD_DEPS_GO` stays default off pending a real dogfood window — see TECHNICAL.md's posture note (#314)
...
### Changed
- test: replayable hook fixtures for the dangling-refs check (#227, phase 2) (#242)
...
### Changed
- fix: fold guard-freshness auto-reinstall into installHooks (#228) (#232)
