...
if echo "$PROMPT_LOWER" | grep -qE '(plan|implement.*feature|build.*new|create.*system|add.*feature|full.*implementation|end.to.end|start.to.finish|from scratch)'; then
...
elif echo "$PROMPT_LOWER" | grep -qE '(architect|design.*system|review.*(security|code|pr|approach)|trade.?off|compare.*approach|strategy|evaluate.*option|assess.*risk|critique|redesign|migrate|overhaul|debug.*complex|root.cause)'; then
