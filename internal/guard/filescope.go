...
func FileScopeViolations(lang Lang, wholeFile []AddedLine, fd FileDiff, repoKnown map[string]struct{}) []Violation {
	...
	// wholeFile is one contiguous run from line 1, so ExtractDefs' own pyBraceDepth
	// tracking computes correctly on it unseeded — the opening `{` of any dict
	// literal is already IN wholeFile. addedLines is not: it's just the hunk, so a
	// key added to an EXISTING dict (opener in unchanged context, outside the
	// hunk) needs the same braceDepthSeedFunc seed extractRefs uses below — without
	// it, ExtractDefs misreads the key as a definition and pyFileScope wrongly
	// binds it into scope, suppressing the exact file-scope violation this check
	// exists to catch (code-review finding on PR #290).
	braceSeed := braceDepthSeedFunc(lang, fd)
	scope := pyFileScope(wholeFile, nil)
	...
	for name := range pyFileScope(addedLines, braceSeed) {
		scope[name] = struct{}{}
	}
	...
	for _, ref := range extractRefs(lang, addedLines, seedFunc(lang, fd), braceSeed) {
...
// across every guard check rather than drifting per-feature. braceDepthSeed is
// threaded straight to extractDefsSeeded — nil is correct for a self-contained,
// contiguous-from-line-1 slice (wholeFile); a hunk slice (addedLines) needs the
// caller's real seed, or a dict key added to an existing literal misreads as a
// definition (see FileScopeViolations).
func pyFileScope(lines []AddedLine, braceDepthSeed func(lineNo int) int) map[string]struct{} {
	scope := make(map[string]struct{})
	for _, n := range ExtractImports(LangPython, lines) {
		scope[n] = struct{}{}
	}
	for _, n := range extractDefsSeeded(LangPython, lines, braceDepthSeed) {
		scope[n] = struct{}{}
	}
