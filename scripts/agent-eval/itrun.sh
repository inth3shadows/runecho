...
# Explore subagent (or drive runecho from the main thread). Only the
...
# Requires: tmux 3.0+, a logged-in `claude` CLI, runecho MCP configured.
set -uo pipefail
...
tmux send-keys -t "$SESSION" "cd $REPO && ${CLAUDE_ENV_PREFIX:-} claude --dangerously-skip-permissions ${CLAUDE_EXTRA_ARGS:-}" Enter
