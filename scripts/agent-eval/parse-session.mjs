...
const RUNECHO_TOOLS = ['locate', 'structure', 'diff', 'hash', 'status', 'health'];
const runechoCounts = {};
let runechoTotal = 0;
for (const t of RUNECHO_TOOLS) {
  const n = (subCounts[`mcp__runecho__${t}`] || 0) + (mainCounts[`mcp__runecho__${t}`] || 0);
  runechoCounts[t] = n; runechoTotal += n;
}
...
const breakdown = RUNECHO_TOOLS.filter(t => runechoCounts[t] > 0).map(t => `${t} ${runechoCounts[t]}`).join(', ') || 'none';
console.log(`\nVERDICT: runecho used ${runechoTotal}x (${breakdown}) | Read ${reads} | Grep/Bash ${greps}`);
